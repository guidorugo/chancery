...
if [ ! -f secrets/pkcs11_user_pin ]; then
  rand_alnum 32 > secrets/pkcs11_user_pin
  chmod 600 secrets/pkcs11_user_pin
  note "created secrets/pkcs11_user_pin (random 32-char alphanumeric)"
else
  note "secrets/pkcs11_user_pin already exists — left unchanged"
fi
if [ ! -f secrets/pkcs11_so_pin ]; then
  rand_alnum 32 > secrets/pkcs11_so_pin
  chmod 600 secrets/pkcs11_so_pin
  note "created secrets/pkcs11_so_pin (random 32-char alphanumeric)"
else
