...
- **Security**: Private keys encrypted at rest with Fernet (PBKDF2-derived key, 600k iterations), session hardening, per-IP rate limiting and per-account login lockout (both on by default), insecure-default rejection plus a startup warning for short `SECRET_KEY` / `MASTER_PASSPHRASE` / PKCS#11 PIN values; the container runs as a non-root user with all capabilities dropped, `no-new-privileges`, a **read-only root filesystem** (tmpfs scratch for gunicorn, everything persistent under `data/`) and an HTTP access log on stdout
...
| `flask users approve <username>` | Break-glass (dual control): approve a pending account or promotion from the shell, audited as `approve_user` by `cli` |
...
| `AUDIT_ARCHIVE_DIR` | `<db dir>/audit-archive` | Where pruned rows are written as JSON lines; must sit under `/app/data` — the container's root filesystem is read-only |
...
| `ACCESS_LOG` | `true` | gunicorn access log to stdout: client, time, method, path (never the query string), status, bytes, latency, user agent — every request, `/health` probes included. `false` switches it off |
...
| `DUAL_CONTROL_COOLDOWN_HOURS` | `24` | While dual control is active: an account created or password-reset by admin X may not approve anything X set up (users, CAs, CA certificates) for this many hours |
